> **UNFROZEN 2026-06-28 — reactivated for forward work.** (Was frozen 2026-06-12 in favour of `../6_WeaponForge_TFTransistor/`; that pivot is unwound for this line of work.) Current effort: a UI/layout reskin — **all 3 heroes present & visible at all times, stacked vertically during forging**, on the **existing 3-slot Head/Hilt/Rune crafting system**. Gameplay logic is UNCHANGED; only UI/layout changes. Layout inspiration: the RuneSurge FTUE 3-hero vertical board (`D:/AI_Workflows/_Claude/RuneSurge/docs/diagrams/`). Active build work happens in the sibling copy (see below once created).
