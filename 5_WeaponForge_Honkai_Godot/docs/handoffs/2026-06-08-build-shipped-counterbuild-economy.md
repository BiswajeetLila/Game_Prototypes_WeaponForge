...
## One-line state
The **pre-stage counter-build loop** and the **Ember economy** are both implemented, tested (301 dev tests green), and pushed on `weaponcraft-godot/wittle-inversion-phase1`. Next planned build = **Catalyst** (element-pair synergy). Owner is playtesting.

## Where to work (exact)
- **Branch:** `weaponcraft-godot/wittle-inversion-phase1` — synced with origin (HEAD ≈ `ddff6b1`+). **Owner-gated: NOT merged to main.**
- **Worktree:** `.claude/worktrees/pedantic-golick-94f7e8/` (warm import cache — the continuity worktree).
- **Godot project:** `5_WeaponForge_Honkai_Godot/Prototype/godot` (boots `scenes/Home.tscn`).
- **Engine ops = godot MCP** (`mcp__godot__run_project`/`get_debug_output`/`stop_project`) at the pedantic-golick path. Dev tests: `run_project(scene="res://scenes/dev/TestX.tscn")` → parse `=== N passed / M failed ===` → `stop_project` (legacy TestCombat doesn't self-quit; stop after summary).
