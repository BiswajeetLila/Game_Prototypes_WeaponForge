...
**Refined 06-22 (built + live-verified):** Keys are now **DIRECTIONAL** — a Key fuses only the tiles its **arrows** point at, so "3 elements on one Key, who wins?" is resolved (a 2-arrow Key simply can't take a 3rd tile; only the epic **Star** fuses 3). Adds the spatial-placement decision the TFT persona wanted. **Stage 4 = 3 Keys** (Bar ◄►, Vee ◣◢ common + Star ★ epic). **Splitter redefined** = directional raw ×2 doubler (no resist-bypass), Sandbox-only. Two playtest UX fixes: a **⚔ Battle button** (combat runs only on press; build freely first) and **Replay now restores enemy HP** (was stuck at 0). All parity-checked, no console errors. **Mana = ticks (new same day):** the bar now holds one **tick per cluster / combo / lone tile**, and the hero fires one group per tick (full attack after the last) — so clustering & combos *consolidate ticks* and the craft drives **tempo as well as damage** (image-1 board = 3 ticks, a Key collapses 2 ticks → 1). Flagged: ~9× throughput swing for clustering → tune enemy HP. See the **two 06-22 docs**. *(#1 OPEN is unchanged — the setup-cost/budget; these don't replace it, the board is still free to build.)*
