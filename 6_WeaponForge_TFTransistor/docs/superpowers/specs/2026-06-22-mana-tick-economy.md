...
- **Fill-rate fix (same day, user-reported):** the bar charged in three 34% jumps per tick (`mana += 34` @640ms), so a 1-cluster board (logically 1 tick) *looked* like it ticked ~3 times. Changed to small steps (`+7` @100ms) → smooth fill. The *count* was always correct (readout said "1 tick"); only the fill granularity was misleading.
- **Final visual (same day, per user diagram) — the bar GROWS:** the mana bar is now **N fixed-width segments, one per tick** — 1 group = 1 short segment, 2 = two segments, 4 = four (a physically *wider* bar that takes longer to fill across). The fill **sweeps across all segments once, then fires the whole board** (fires once at the end — framing B). This supersedes both the per-segment-firing first build *and* the single-fixed-width-bar interim: it is one continuous charge, just drawn as a growing row of tick-segments so the player can *see* the tick count. Verified live: 1 / 2 / 4 groups → 1 / 2 / 4 segments; a 2-segment board sweeps both segments then fires 12 (whole board), looping; no console errors. **This is the canonical mana visual.**
...
## 7. Clustering's damage bonus (cmult) CUT (2026-06-22)
**Decision (after a brutal analysis the user requested):** removed the cluster **adjacency damage bonus** (`cmult`, was ×1.0–1.9). Clustering now does **only** two things: (a) **groups adjacent same-element tiles into one tick** (the tempo spine), and (b) **enables merge** (3 adjacent identical → tier-up). **Merge owns damage scaling** (×4/×9 by tier), alongside count and combos.

**Why (the analysis):**
- Engine numbers showed clustering's value was ~90% *tempo*, ~10% *damage*: "spread" 3 fire = 22 dmg / 2 ticks (11/s) vs clustered = 29 / 1 tick (29/s) — the jump was the tick, not the +32% cmult. Merge dwarfs cmult: 29 → 68 → 148.
- `cmult` was a 4th stacked damage multiplier (count × cmult × tier × combo) — redundant with merge and extra to learn.
- It wasn't even a *decision*: the 5-cell hex's max non-adjacent same-element set is **2** (only A0+A2), so you literally can't spread 3 of a kind — adjacency is forced; and clustering had no downside. A mechanic with no alternative and no cost is plumbing, not strategy.

**Result:** `baseOut = 6 × TIERMULT[tier]` (×2 if a Splitter points at it). Clustered tiles do the same per-tile damage as lone ones — they just fire as one (faster) tick and can be merged. Verified live + engine: 3 clustered 29→**18**, 5 clustered 53→**30**, merge T2+refill 68→**36**; Key parity intact (Bar 36 / Star 54, their tiles weren't clustered). **Implication:** mono-board damage dropped ~30–60% → folds into the pending #1 HP retune; and merging is now *more* important relative to bare clustering (intended). Stage-2 copy reframed from "damage engine / hits harder" to "fires as one tick / can be merged."
