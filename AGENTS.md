...
> Scope: applies to **all** sibling Godot prototype folders in this repo. Each sub-project may override or extend via its own `<sub-project>/AGENTS.md`.
...
This monorepo's branch-naming convention follows the global policy in `~/.Codex/AGENTS.md` ("Branch naming policy"). The sub-project slugs to use here:
...
Random-slug rename: if tooling forces `Codex/<adj>-<scientist>-<hex>`, immediately `git branch -m <slug>/<keywords>` before doing meaningful work.
