...
- **Fill-rate fix (same day, user-reported):** the bar charged in three 34% jumps per tick (`mana += 34` @640ms), so a 1-cluster board (logically 1 tick) *looked* like it ticked ~3 times — the visible charging didn't match the stated "1 tick." Changed to small steps (`+7` @100ms) → **one smooth fill = one tick**. Re-verified: a 5-Fire cluster fills once → `⚔ ATTACK → 30` (no intermediate `tick` messages); a 2-tick board still shows one `tick 1/2 charged` then fires. The *count* was always correct (readout said "1 tick"); only the fill granularity was misleading.
